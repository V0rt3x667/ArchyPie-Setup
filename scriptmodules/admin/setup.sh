...
function uninstall_setup() {
    ...
    rm -rfv "$rootdir"
    dialog --defaultno --yesno "Do you want to remove all the files from $datadir? \n\nWARNING: This includes all your installed ROMs, BIOS files and custom splashscreens." 22 76 2>&1 >/dev/tty && rm -rfv "$datadir"
...
function gui_setup() {
    ...
    while true; do
        ...
        options=(
            I "Basic Install"
           "I This will install all packages from Core and Main which gives a basic ArchyPie install. Further packages can then be installed later from the Optional and Experimental sections. If binaries are available they will be used, alternatively packages will be built from source which will take longer."

            U "Update"
            "U Updates ArchyPie-Setup and all currently installed packages. Will also allow to update OS packages. If binaries are available they will be used, otherwise packages will be built from source."
