...
function config_gui_setup() {
    ...
    while true; do
        ...
        local id
        for id in "${__mod_id[@]}"; do
            # Show all configuration modules & any installed packages with a GUI function
...
function uninstall_setup() {
    ...
    dialog --defaultno --colors --yesno "Are you \ZbREALLY\Zn sure you want to uninstall ArchyPie?\n\nThe \Z4\Zb${rootdir}\Zn folder will be removed - this includes configuration files for all ArchyPie components. If you wish to keep any config files of the emulators/ports that ArchyPie used, then choose \ZrNo\Zn now & save it first." 22 76 2>&1 >/dev/tty || return 0
    clear
    ...
    rm -rfv "${rootdir}"
    dialog --defaultno --yesno "Do you want to remove all the files from ${datadir}? \n\nWARNING: This includes all your installed ROMs, BIOS files & custom splashscreens." 22 76 2>&1 >/dev/tty && rm -rfv "${datadir}"
