...
function build_lr-puae() {
    local binary="${1}"
    [[ -z "${binary}" ]] && binary="puae_libretro.so"
...
function configure_lr-puae() {
    ...
    if [[ "${md_mode}" == "install" ]]; then
        for system in "${systems[@]}"; do
            ...
            # Symlink supported systems BIOS directories to the ${biosdir}/amiga folder
        ...
        # Copy CAPs image & floppy disk audio files to the ${biosdir}/amiga folder
    ...
    for system in "${systems[@]}"; do
        addEmulator 1 "${md_id}" "${system}" "${md_inst}/${binary}"
        addSystem "${system}"
    done
}
