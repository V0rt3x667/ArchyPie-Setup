...
function pacmanPkg() {
    ...
    for pkg in $PKGBUILD; do
        ...
        BUILDDIR='"$md_build"' \
        PKGDEST='"$md_build/$pkg"' \
        SRCDEST='"$md_build/$pkg"' \
        SRCPKGDEST='"$md_build/$pkg"' \
...
## user is archypie, host is $__binary_host and default port is 22. These can be overridden with
