...
function build_ppsspp() {
    ...
    if isPlatform "rpi"; then
        ...
        # remove -DGL_GLEXT_PROTOTYPES on odroid-xu to avoid errors due to header prototype differences
...
function configure_ppsspp() {
    ...
    addEmulator 1 "$md_id" "psp" "pushd $md_inst; $md_inst/bin/PPSSPPSDL ${extra_params[*]} %ROM%; popd"
    addSystem "psp"
