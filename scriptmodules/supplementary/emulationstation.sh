...
function build_emulationstation() {
    ...
    if isPlatform "rpi"; then
        ...
        if isPlatform "gles"; then
            params+=('-DGLES=ON')
            local gles_ver=$(sudo -u "${__user}" eglinfo -B | grep -m 1 'OpenGL ES profile version:' | cut -d" " -f7)
            ...
            local gl_ver=$(sudo -u "${__user}" eglinfo -B | grep -m 1 'OpenGL compatibility profile version:' | cut -d" " -f5)
...
_EOF_
    ...
    if isPlatform "x11"; then
        mkdir -p /usr/local/{icons,applications}
        cp "${scriptdir}/scriptmodules/${md_type}/emulationstation/retropie.svg" "/usr/local/icons/"
        cat > /usr/local/applications/archypie.desktop << _EOF_
[Desktop Entry]
Type=Application
...
Icon=/usr/local/icons/retropie.svg
Categories=Game
_EOF_
    fi
}
...
function remove_emulationstation() {
    ...
    if isPlatform "x11"; then
        rm -rfv "/usr/local/icons/retropie.svg" "/usr/local/applications/archypie.desktop"
    fi
}
