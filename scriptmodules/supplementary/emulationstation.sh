...
function depends_emulationstation() {
    ...
    isPlatform "x11" && depends+=('gnome-terminal' 'libsm' 'mesa-utils')
    getDepends "${depends[@]}"
}
...
if [[ \$? -eq 139 ]]; then
    dialog --cr-wrap --no-collapse --msgbox "EmulationStation crashed!\n\nIf this is your first boot of ArchyPie, make sure you are using the correct image for your system.\n\\nCheck your rom file/folder permissions & if running on a Raspberry Pi, make sure your gpu_split is set high enough and/or switch back to using the Carbon theme." 20 60 >/dev/tty
fi
tput cnorm
_EOF_
