...
function depends_xboxdrv() {
    getDepends boost libusb libudev libx11 scons pkg-config dbus-glib dbus-python
}
...
function enable_xboxdrv() {
    ...
    cat > /usr/lib/systemd/system/xboxdrv.service << _EOF_
[Unit]
...
ExecStart=$md_inst/bin/xboxdrv --daemon --detach --pid-file /var/run/xboxdrv.pid -c /etc/default/xboxdrv --dbus disabled --detach-kernel-driver
...
_EOF_
    ...
    if grep -q "xboxdrv" /etc/default/xboxdrv; then
        dialog --yesno "xboxdrv is already enabled with the following config. Do you want to update it ?\n\n$(grep "xboxdrv" /etc/default/xboxdrv)" 22 76 2>&1 >/dev/tty || return
    fi

    sed -i "/xboxdrv/d" /etc/default/xboxdrv
    sed -i "s|^exit 0$|${config}\\nexit 0|" /etc/default/xboxdrv
    systemctl enable xboxdrv.service && systemctl start xboxdrv.service
    printMsgs "dialog" "xboxdrv service enabled and started with the following config\n\n$config"
}

function disable_xboxdrv() {
    sed -i "/xboxdrv/d" /etc/default/xboxdrv
    systemctl stop xboxdrv.service && systemctl disable xboxdrv.service
    printMsgs "dialog" "xboxdrv service stopped and disabled. The configuration in /etc/default/xboxdrv has been removed."
}
...
function configure_xboxdrv() {
    ...
    sed -i "s|^xboxdrv|\"$md_inst/bin/xboxdrv\"|" /etc/default/xboxdrv
}
