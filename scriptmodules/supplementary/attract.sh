...
function _get_configdir_attract() {
    echo "${configdir}/all/${md_id}"
}

function _add_system_attract() {
    local attract_dir="$(_get_configdir_attract)"
    [[ ! -d "${attract_dir}" || ! -f "/usr/bin/${md_id}" ]] && return 0
    ...
    if [[ ! -f "${attract_dir}/romlists/${fullname}.txt" ]] && [[ -f "/usr/bin/${md_id}" ]]; then
        sudo -u "${__user}" ${md_id} --build-romlist "${fullname}" -o "${fullname}"
    else
        return
    fi
...
function depends_attract() {
    ...
    if [[ "${md_id}" == "attractplus" ]]; then
        depends+=('boost')
    fi
    getDepends "${depends[@]}"
}

function sources_attract() {
    gitPullOrClone
...
function remove_attract() {
    rm -f "/usr/bin/${md_id}"
}

function configure_attract() {
