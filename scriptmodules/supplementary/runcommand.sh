...
function install_bin_runcommand() {
    ...
    if [[ ! -f "$configdir/all/runcommand.cfg" ]]; then
        ...
        hasFlag "nosysd" && iniSet "legacy_joy2key" "1"
...
function gui_runcommand() {
    ...
    while true; do
        ...
        if [[ "$legacy_joy2key" -eq 1 ]]; then
            options+=(6 "Use Legacy Joy2Key (Currently: Enabled)")
        else
            options+=(6 "Use Legacy Joy2Key (Currently: Disabled)")
        fi
