...
function basic_configedit() {
    ...
    local ini_descs=(
        ...
        'Configure the resolution to render the emulator output at - for better performance on full HD displays choose a lower resolution & it will be upscaled in hardware'
...
function advanced_configedit() {
    ...
    local ini_descs=(
        ...
        'Aspect ratio to use (default unset - will use core aspect if video_aspect_ratio_auto is true)'
        'Load video_shader on startup. Other shaders can still be loaded later in runtime.'
        'Video shader to use. Video driver gl supports *.glslp shader presets. Video drivers glcore & vulkan support *.slangp shader presets. (default none)'
        'Load input overlay on startup. Other overlays can still be loaded later in runtime.'
        'Input overlay to use (default none)'
        ...
        'Use threaded video driver. Using this might improve performance at possible cost of latency & more video stuttering.'
        'Forces rendering area to stay equal to content aspect ratio or as defined in video_aspect_ratio.'
        'Only scales video in integer steps. The base size depends on system-reported geometry & aspect ratio. If video_force_aspect is not set, X/Y will be integer scaled independently.'
        'If this is true & video_aspect_ratio or video_aspect_ratio_index is not set, aspect ratio is decided by libretro implementation. If this is false, 1:1 PAR will always be assumed if video_aspect_ratio or  video_aspect_ratio_index is not set.'
...
function advanced_menu_configedit() {
    while true; do
        ...
        local options=(
            ...
            3 "Manually Edit Global Configurations"
